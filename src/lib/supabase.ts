...
// Add a function to get all payments with pagination
export const getAllPaymentsPaginated = async (
  ...
  try {
    console.log(`Fetching paginated payments for page ${page}, pageSize ${pageSize}, month ${month || 'all'}`);
    ...
    console.log(`Fetched ${data?.length} payments, total: ${count}`);
    return { 
      payments: data as Payment[] || [], 
      total: count || 0 
    };
  } catch (error) {
    console.error("Error in getAllPaymentsPaginated:", error);
    return { payments: [], total: 0 };
  }
};

// Add a function to get paginated cash withdrawals
export const getCashWithdrawalsPaginated = async (
  page: number = 1,
  pageSize: number = 20,
  month?: string
): Promise<{ withdrawals: CashWithdrawal[], total: number }> => {
  try {
    console.log(`Fetching paginated withdrawals for page ${page}, pageSize ${pageSize}, month ${month || 'all'}`);
    ...
    console.log(`Fetched ${data?.length} withdrawals, total: ${count}`);
    return { 
      withdrawals: data as CashWithdrawal[] || [], 
      total: count || 0 
    };
  } catch (error) {
    console.error("Error in getCashWithdrawalsPaginated:", error);
